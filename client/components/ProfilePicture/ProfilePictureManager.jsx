...
const ProfilePictureManager = ({
  ...
  const getImageUrl = () => {
    ...
    if (currentImageUrl.includes('BlankProfilePicture') || 
        currentImageUrl.includes('BlankBabyPicture')) {
  ...
  const handleImageUpload = async () => {
    ...
    if (!entityId) {
      ...
      console.error('Missing entityId in ProfilePictureManager:', {entityType, entityId});
      return;
    }
  ...
  const handleRemoveImage = async () => {
    if (!window.confirm(t('Are you sure you want to remove this profile picture?'))) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || t('Failed to remove profile picture.'));
      }
        ...
        <div className={styles.buttonContainer}>
          ...
          {!isDefaultImage && (
            <Button 
              ...
              disabled={uploading}
            >
              {uploading ? <Spinner animation="border" size="sm" /> : t('Remove')}
            </Button>
          )}
        </div>
      )}
      ...
      >
              ...
              <div className={styles.cropContainer}>
                ...
                {!fileLoading && (
                  <ReactCrop
                    ...
                    crop={crop}
                    onChange={(newCrop) => {
                      const boundedCrop = ensureCropInBounds(newCrop, imgRef.current?.width, imgRef.current?.height);
                      setCrop(boundedCrop);
                    }}
                    onComplete={(c) => {
                      const boundedCrop = ensureCropInBounds(c, imgRef.current?.width, imgRef.current?.height);
        ...
        <Modal.Footer className="bg-white">
          ...
          <Button
            ...
            disabled={!selectedFile || !completedCrop || uploading || fileLoading}
          >
            {uploading ? (
              <>
