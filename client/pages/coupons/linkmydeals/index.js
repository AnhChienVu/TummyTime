...
const LinkMyDealsPage = () => {
  ...
  const categories = [...new Set(offers.flatMap(offer => 
    offer.categories ? offer.categories.split(',').map(c => c.trim()) : []
  ))].filter(Boolean).sort();
        ...
        <div className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>
              {t("Browse the latest deals and coupons from thousands of online stores")}
            </p>
          </div>
            ...
            <div className={styles.filtersRow}>
              ...
              <div className={styles.filterButtons}>
                <Button 
                  ...
                  disabled={!searchTerm && !selectedType && !selectedCategory && !selectedStore}
                >
                  {t("Reset Filters")}
                </Button>
              </div>
          ...
          <div className={styles.resultsSummary}>
            <p>
              {filteredOffers.length === 0 
                ...
                : t("Showing {{count}} offers", { count: filteredOffers.length })}
              {(searchTerm || selectedType || selectedCategory || selectedStore) && 
                ...
                <div className={styles.couponCard}>
                  <div className={styles.cardImageWrapper}>
                    {offer.image_url ? (
                      <img
                        src={offer.image_url}
                        alt={offer.title || offer.offer_text}
                  ...
                  <div className={styles.cardBody}>
                    <div className={styles.cardContent}>
                      <div className={styles.storeDetails}>
                        <span className={styles.storeName}>{offer.store || t("Online Store")}</span>
                        <span className={styles.category}>
                          {offer.categories ? offer.categories.split(',')[0] : ""}
                        </span>
                      </div>
                    ...
                    <div className={styles.cardFooter}>
                      {offer.code ? (
                        <>
                          ...
                          <a
                            href={offer.smartLink || offer.url || offer.merchant_homepage}
                            target="_blank"
                      ...
                      {offer.end_date && (
                        <div className="text-center mt-2">
                          <small className="text-muted">
                            {t("Expires")}: {new Date(offer.end_date).toLocaleDateString()}
                          </small>
                        </div>
                      )}
                    </div>
