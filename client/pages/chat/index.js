...
import { FaPlus, FaComment, FaTrash, FaUser, FaInfoCircle } from 'react-icons/fa';
...
const generateSmartTitle = (userQuery, botResponse = '') => {
  ...
  const topics = {
    ...
    'Feeding & Nutrition': ['feed', 'formula', 'breast', 'milk', 'bottle', 'eat', 'eating', 'food', 'solid', 'nurse', 'hungry', 'appetite', 'nutrition'],
    'Development': ['milestone', 'crawl', 'walk', 'talk', 'roll', 'sit', 'stand', 'development', 'grow', 'skill', 'motor'],
    'Health Concern': ['sick', 'fever', 'temperature', 'cold', 'cough', 'rash', 'vomit', 'diarrhea', 'constipation', 'doctor', 'medicine', 'vaccine'],
    'Behavior & Routine': ['cry', 'crying', 'fussy', 'tantrum', 'behavior', 'calm', 'soothe', 'habit', 'routine', 'discipline'],
    'Teething': ['teeth', 'tooth', 'teething', 'gum', 'drool'],
    'Diapering & Potty': ['diaper', 'poop', 'pee', 'stool', 'urine', 'bowel', 'toilet', 'potty']
  };

  let agePrefix = '';
  const ageMatch = normalizedQuery.match(/(\d+)\s*(month|week|year|day)s?\s*(old)?/);
  if (ageMatch) {
    agePrefix = `${ageMatch[1]} ${ageMatch[2]}${parseInt(ageMatch[1]) !== 1 ? 's' : ''}: `;
  }
  ...
  let cleanedQuery = normalizedQuery
    .replace(/^(is|are|the|about|how|what|when|why|do|does|can|could|should|would)\s+/i, '')
...
const ChatPage = () => {
  ...
  const loadConversations = () => {
    ...
    if (storedConversations) {
      try {
        ...
        localStorage.setItem('babyAssistantConversations', JSON.stringify(parsedConversations));
  ...
  const sendMessage = async (e) => {
    ...
    if (!inputText.trim()) {
      const emptyMessage = { text: "Your message was empty. Please try again.", isBot: true };
    ...
    if (!activeConversation) {
      ...
      showNotification('New conversation created. Please try sending your message again.');
      return;
    }
    ...
    let currentConversation = conversations.find(conv => conv.id === activeConversation);
    ...
    try {
      ...
      if (!isRelevantQuery(currentInput)) {
        botReply = "I'm specialized in helping with questions about babies and children ages 0-7. " +
        ...
        botReply = typeof response === 'object' && response.responseText 
      ...
      setConversations(updatedConversations);
      showNotification('Error connecting to the assistant. Please try again later.');
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Chat with our Baby Care Assistant for answers about infant and child care" />
      </Head>
              ...
              >
                <div className={styles.conversationDetails}>
                  ...
                  <div className={styles.conversationText}>
                    ...
                    <p>{new Date(conversation.lastUpdated).toLocaleDateString()}</p>
                  </div>
                </div>
                <button 
