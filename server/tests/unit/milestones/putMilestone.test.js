...
app.put(
  '/v1/baby/:baby_id/updateMilestone/:milestone_id',
  authenticate(),
  updateMilestoneById
);
...
describe('PUT /v1/baby/:baby_id/updateMilestone/:milestone_id', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test('should return 200 and update the milestone record', async () => {
    ...
    const res = await request(app)
      .put('/v1/baby/1/updateMilestone/1')
  ...
  test('should return 404 if the milestone record is not found', async () => {
    ...
    const res = await request(app)
      .put('/v1/baby/1/updateMilestone/1000')
      .set('Authorization', `Bearer ${token}`) // Include the token in the Authorization header
      .send({
        date: '2025-02-20',
        title: 'First Walk',
